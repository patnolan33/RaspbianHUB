...
class Trie {
  ...
  suggest(string: string) {
    ...
    this.selectedWords.forEach((word) => {  // loop over the selectedWords array
      if ((suggestions as any).includes(word)) {  //  if any word is included in the suggestions
  ...
  findWords(node: Node, prefix: string[] = [], wordCompletions: string[] = []) {
